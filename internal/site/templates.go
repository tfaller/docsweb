...
// searchTmpl is the site-wide "Search" tab's static shell: a mount point
// for pagefind's own prebuilt UI bundle (pagefind-ui.js/.css), not anything
// compiled through web/'s TS pipeline - those files are generated by the
// pagefind binary itself against this build's output directory, after
// Generate has finished writing every page (see cmd/docsweb's runBuild and
// writeSearchPage below), so they can't be embedded into the docsweb binary
// the way bundle.js is.
var searchTmpl = template.Must(template.New("search").Parse(`
<h1>Search</h1>
<p>Full-text search across every page on this site, powered by
